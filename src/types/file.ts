...
/**
 * Interface representing a file column with its properties
 */
interface Column {
  /** Column name */
  name: string
...
export default class File extends Base {
  ...
  /**
   * Fetch columns from QSYS2.SYSCOLUMNS
   * Retrieves column definitions including data types, lengths, and default values
   */
    ...
    if (connection) {
      ...
      const entryRows = await executeSqlIfExists(
        ...
        `SELECT COLUMN_NAME,
            ...
            LENGTH,
            "CCSID",
            case when IS_NULLABLE='N' THEN 'NO' ELSE 'YES' END AS IS_NULLABLE,
            CASE HAS_DEFAULT
                WHEN 'N' THEN 'NO'
                WHEN 'Y' THEN 'YES'
                WHEN 'A' THEN 'ROWID GENERATED ALWAYS'
                WHEN 'D' THEN 'ROWID GENERATED BY DEFAULT'
                WHEN 'E' THEN 'FOR EACH ROW ON UPDATE - GENERATED ALWAYS'
                WHEN 'F' THEN 'FOR EACH ROW ON UPDATE - GENERATED BY DEFAULT'
                WHEN 'I' THEN 'IDENTITY - GENERATED ALWAYS'
                WHEN 'J' THEN 'IDENTITY - GENERATED BY DEFAULT'
                WHEN 'Q' THEN 'GENERATED AS ROW BEGIN'
                WHEN 'R' THEN 'GENERATED AS ROW END'
                WHEN 'X' THEN 'GENERATED AS TRANSACTION START ID'
                WHEN 'A' THEN 'GENERATED EXPRESSION USING SPECIAL REGISTER'
                WHEN 'C' THEN 'GENERATED EXPRESSION USING GLOBAL VARIABLE'
                WHEN 'D' THEN 'GENERATED EXPRESSION USING DATA CHANGE OPERATION'
                ELSE 'UNKNOWN'
            END AS HAS_DEFAULT,
            COLUMN_DEFAULT,
            COLUMN_TEXT
      ...
      this.fileColumns.push(...entryRows.map(this.toColumn));
    } else {
      vscode.window.showErrorMessage(vscode.l10n.t("Not connected to IBM i"));
      return;
    }
  ...
  /**
   * Convert a database row to a Column object
   * @param row - Database row from QSYS2.SYSCOLUMNS
   * @returns Column object representing a file column
   * @private
   */
  private toColumn(row: Tools.DB2Row): Column {
    return {
      ...
      hasdft: String(row.HAS_DEFAULT),
      dft: String(row.COLUMN_DEFAULT),
      text: String(row.COLUMN_TEXT)
    };
  }
  /**
  ...
  /**
   * Render the file columns table
   * Displays all columns of the file with their properties
   * @returns HTML string for the fast table component
   * @private
   */
  renderFileColumns() {
    ...
    const columns: FastTableColumn<Column>[] = [
      ...
      { title: vscode.l10n.t("CCSID"), width: "0.5fr", getValue: e => e.ccsid },
      { title: vscode.l10n.t("Is nullable"), width: "0.5fr", getValue: e => e.nullable },
      { title: vscode.l10n.t("Has default"), width: "1fr", getValue: e => e.hasdft },
      { title: vscode.l10n.t("Default"), width: "1fr", getValue: e => e.dft },
      { title: vscode.l10n.t("Text"), width: "2fr", getValue: e => e.text }
    ];
